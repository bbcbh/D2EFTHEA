...
public class Runnable_SingleQALYComparsion implements Runnable {
	...
	private ArrayList<Float>[][][][] healthUtil_REF_USAGE = new ArrayList[XLSX_Extract_HEA.STUDY_ARM.length][XLSX_Extract_HEA.VISIT_NUM.length][][];
	...
	public void run() {
		...
		for (int s = 0; s < mapping_study_arm_offset.length; s++) {
			...
			float[] healthUtil_by_visit_arm_day_0 = sampleHealthUtil(rng, s, 0, sampleVal);
			for (int i = 0; i < interpol_HealthUtil.length; i++) {
				interpol_HealthUtil[i][0] = healthUtil_by_visit_arm_day_0[i];
			}
			...
			for (int vPt = 0; vPt < visit_to_sample.length; vPt++) {
				...
				while (posToInclude[0] > 0 && (vaild_qaly_mapping[posToInclude[0] - 1][visitResPt]) >= Math.max(0,
				...
				float[] healthUtil_by_visit_arm = sampleHealthUtil(rng, s, visitResPt,
						interpol_QALY[visitResPt]);
				for (int i = 0; i < interpol_HealthUtil.length; i++) {
					interpol_HealthUtil[i][visitResPt] = healthUtil_by_visit_arm[i];
				}
	...
	private float[] sampleHealthUtil(RandomGenerator rng, int study_arm, int visit, float qaly) {
		...
		int endIndex = Arrays.binarySearch(healthUtil_qalys, qaly + DEFAULT_DELTA_QALY_RANGE);
		if (endIndex < 0) {
			endIndex = ~endIndex;
		}
		...
		float[] healthUtil_by_visit_arm = new float[RunSimulations.NUM_HEALTHUTIL_RESP];
